	...
	<div v-if="isOpen" @click="cerrarBackdrop" class="backdrop-blur-[2px] bg-gray-200/50 fixed inset-0 overflow-y-auto flex justify-center items-center">
		<div class="bg-white p-4 rounded-lg shadow-xl w-175 border-2 border-gray-300">
			<div class="mb-3 flex items-center justify-between">
				<span class="text-xl font-semibold">Agregar Cliente</span>
			...
			<div>
				<div class="grid grid-cols-2 gap-2">
					...
					<button class="rounded rounded-lg font-bold p-3 text-md text-green-500 hover:scale-101 transition hover:bg-green-500 hover:outline-0 hover:text-white hover:shadow-lg active:scale-[0.98] ease-in-out cursor-pointer">Agregar Cliente</button>
				</div>
			</div>
		</div>
	</div>
